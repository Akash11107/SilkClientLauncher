...
berry.log("Starting Silk Client Launcher...");
...
function checkForUpdates() {
  return new Promise((resolve, reject) => {
     ...
     */
    axios.get("https://api.github.com/repos/SilkClientLauncher/launcher/releases").then(async (r) => {
...
function update(url) {
  return new Promise((resolve, reject) => {
    axios({
      ...
      // You can't replace.asar!
      try {
      response.data.pipe(createWriteStream(join(tempDir, "app.SilkClient")))
        .on("finish", () => {
          copyFile(join(tempDir, "app.SilkClient"), join(tempDir, "app.asar"), (err) => {
            if (err) {
              ...
              resolve(true);
              unlinkSync(join(tempDir, "app.SilkClient"));
            }
          });
          resolve(true);
        })
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    skipTaskbar: true,
    title: "Silk Client Launcher",
    fullscreenable: false,
    icon: join(__dirname, "assets/images/silkclient-launcher-icon.png"),
    transparent: true,
    webPreferences: {
      spellcheck: false,
  ...
  require("./gameManager")(mainWindow);
  require("./versionManager")(mainWindow);
  require("./accountManager")(mainWindow);
};
